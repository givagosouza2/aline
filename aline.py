    ...
    for index, valor in enumerate(rms_curve_x):
        if valor > min + max*0.01:
            ...
            if contador > 2:
                start = index
                break
    escolha = st.checkbox('Quer modelar os dados?')
    # modelo do começo
...
# modelo do começo
    ...
    with col1:
        ...
        ax.plot([tempo_rms_x[start], tempo_rms_x[start]], [0, 3], 'k')
        ax.plot([tempo_rms_x[start]+1500, tempo_rms_x[start]+1500], [0, 3], 'k')
